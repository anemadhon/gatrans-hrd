...
class Tepe extends CI_Controller {
	...
	public function display($from=0){
		if ($this->session->has_userdata('hasadmin')) {
			$jmlData = $this->DisplayData->showDataTP()->num_rows();
			$this->load->library('pagination');
			$config['base_url'] = site_url().'/data-tp/display/';
	        ...
	        $config['cur_tag_open'] = '<a href="http://hrd.gatrans.net/data-tp/display/" class="active">';
			...
			$sesiLog = array(
						...
						'tepe' => $this->DisplayData->showDataTP($config['per_page'],$from)
					);
